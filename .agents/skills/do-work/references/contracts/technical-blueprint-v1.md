...
Before each exact-artifact review and before approval, run `scripts/validate_planning_artifact.py --stage review` or `--stage approval` with the artifact and recorded expected baseline revision. These final stages require `status: complete` and no unresolved decisions; syntax-valid metadata is insufficient. The gate recomputes `content_hash`, resolves `baseline_sha` locally, and fails closed when expected revision context is absent or either check fails. Use `--stage advisory` only for explicitly requested non-final validation of an intermediate draft.
