...
`contracts/planning-result-v1.json` is the versioned machine-readable contract for repository mapping and planning evidence. A result records `schema_version`, `kind: planning-result`, `role: map-codebase`, `status`, `artifact_id`, the immutable `baseline_sha`, a canonical `content_hash` (`sha256:<hex>`), `repository_map.unknowns` (explicitly `[]` when none exist), unresolved decisions, and explicit acceptance and verification mappings.
